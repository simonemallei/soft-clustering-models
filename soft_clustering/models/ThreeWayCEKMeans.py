...
class ThreeWayCEKMeans(BaseEstimator):
    """ An estimator using Three-Way CE K-Means algorithm proposed in:
    ...
    """
    def __init__(self, n_iter=30, n_clusters=10, init='k-means++', q=15, rho=1.5, random_state=42):
        self.n_iter = n_iter
    ...
    def __kmeans(self, X):
        ...
        while stop != True:
            ...
            for ind in range(self.n_clusters):
                if np.sum(y[:, ind]) > 0.0:
                    center = np.sum(X * (y[:, ind].reshape(X.shape[0], 1)),
                                    axis=0) / np.sum(y[:, ind])
